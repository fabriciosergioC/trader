...
/**
 * Componente Super DOM Panel
 * Exibe o livro de ordens visual com 5 níveis de bid/ask,
 * termômetro de imbalance, POC, Value Area, absorção e iceberg.
 */
const SuperDomPanel = ({ domAnalysis, preco }) => {
    ...
    // Configuração de cores para o imbalance
    ...
    const topBuckets = useMemo(() => {
        ...
        return volumeProfile.topBuckets
            .sort((a, b) => b.priceMid - a.priceMid) // Ordenar por preço (maior em cima)
            .slice(0, 8)
            .map(b => ({
                ...b,
                barWidth: maxVol > 0 ? (b.volumeTotal / maxVol) * 100 : 0
            }));
    }, [volumeProfile]);

    return (
        <div className="dom-section">
            <h3 className="dom-title">📊 Super DOM — Livro de Ordens</h3>
            ...
            <div className="dom-book">
                        ...
                        <div key={`ask-${i}`} className={`dom-level ask ${level.isPOC ? 'poc' : ''} ${level.isVALevel ? 'va' : ''}`}>
                            <div className="dom-vol-ask">
                                <div className="dom-bar-bg">
                                    <div className="dom-bar-fill ask" style={{ width: `${barW}%` }} />
                                </div>
                                <span className="dom-vol-label">{level.volume?.toLocaleString('pt-BR')}</span>
                            </div>
                            <div className="dom-price-cell ask">
                                R$ {fmt(level.price)}
                                {level.isPOC && <span className="dom-poc-badge">POC</span>}
                                {level.isVALevel && !level.isPOC && <span className="dom-va-badge">VA</span>}
                            </div>
                ...
                <div className="dom-current-price">
                    ...
                    <div className="dom-cp-value">R$ {fmt(preco)}</div>
                    <div className="dom-cp-loc">{priceLocation?.replace('_', ' ')}</div>
                </div>

                {/* BID levels — do mais alto para mais baixo */}
                {bids.map((level, i) => {
                    const barW = maxVolume > 0 ? (level.volume / maxVolume) * 80 : 0;
                    return (
                        <div key={`bid-${i}`} className={`dom-level bid ${level.isPOC ? 'poc' : ''} ${level.isVALevel ? 'va' : ''}`}>
                            ...
                            <div className="dom-vol-bid">
                                <span className="dom-vol-label">{level.volume?.toLocaleString('pt-BR')}</span>
                                <div className="dom-bar-bg">
                                    <div className="dom-bar-fill bid" style={{ width: `${barW}%` }} />
                                </div>
                            </div>
                        </div>
                    );
            ...
            {/* ─── TERMÔMETRO DE IMBALANCE ─── */}
            <div className="dom-imbalance-section">
                <div className="dom-imb-labels">
                    <span style={{ color: '#EF4444' }}>VENDA {totalAskVolume?.toLocaleString('pt-BR')}</span>
                    <span className="dom-imb-title">IMBALANCE DO BOOK</span>
                    <span style={{ color: '#10B981' }}>COMPRA {totalBidVolume?.toLocaleString('pt-BR')}</span>
                </div>
                <div className="dom-imb-bar-bg">
                    <div
                        className="dom-imb-bar-fill"
                        style={{
                            width: `${imbalancePct}%`,
                            marginLeft: imbalance >= 0 ? '50%' : `${50 - imbalancePct}%`,
                            background: imbalanceColor
                        }}
                    />
                ...
                <div className="dom-va-section">
                    {poc && (
                        <div className="dom-va-item">
                            ...
                            <span className="dom-va-val">R$ {fmt(poc.price)}</span>
                    ...
                    {valueArea && (
                        <>
                            <div className="dom-va-item">
                                ...
                                <span className="dom-va-val" style={{ color: '#EF4444' }}>R$ {fmt(valueArea.vaHigh)}</span>
                            </div>
                            <div className="dom-va-item">
                                <span className="dom-va-lbl">VA Low</span>
                                <span className="dom-va-val" style={{ color: '#10B981' }}>R$ {fmt(valueArea.vaLow)}</span>
                            </div>
                        </>
                    )}
                </div>
            ...
            {/* ─── ALERTAS: ABSORÇÃO E ICEBERG ─── */}
            {(absorption?.hasRecentAbsorption || iceberg?.hasIceberg) && (
                <div className="dom-alerts">
                    {absorption?.hasRecentAbsorption && (
                        <div className={`dom-alert ${absorption.absorptionType === 'ABSORÇÃO_COMPRA' ? 'buy' : 'sell'}`}>
                            <span className="dom-alert-icon">🏦</span>
                            <div className="dom-alert-text">
                                <strong>{absorption.latestAbsorption?.type}</strong>
                                <span>{absorption.latestAbsorption?.interpretation}</span>
                            </div>
                        </div>
                    )}
                    {iceberg?.hasIceberg && iceberg.latestIceberg && (
                        <div className={`dom-alert ${iceberg.latestIceberg.type === 'ICEBERG_COMPRA' ? 'buy' : 'sell'}`}>
                            <span className="dom-alert-icon">🧊</span>
                            <div className="dom-alert-text">
                                <strong>{iceberg.latestIceberg.type}</strong>
                                <span>{iceberg.latestIceberg.interpretation}</span>
                            </div>
                        </div>
                    )}
                </div>
            ...
            {/* ─── VOLUME PROFILE (TOP BUCKETS) ─── */}
            {topBuckets.length > 0 && (
                <div className="dom-vp-section">
                    <div className="dom-vp-title">Volume Profile (Nível × Volume)</div>
                    {topBuckets.map((b, i) => {
                        const isPoC = poc && Math.abs(b.priceMid - poc.price) < 0.01;
                        const isVA = valueArea && b.priceMid >= valueArea.vaLow && b.priceMid <= valueArea.vaHigh;
                        return (
                            <div key={i} className={`dom-vp-row ${isPoC ? 'poc' : isVA ? 'va' : ''}`}>
                                <span className="dom-vp-price">R$ {fmt(b.priceMid)}</span>
                                <div className="dom-vp-bar-bg">
                                    <div className="dom-vp-bar-fill" style={{ width: `${b.barWidth}%` }} />
                                </div>
                                {isPoC && <span className="dom-poc-tag">POC</span>}
                                {isVA && !isPoC && <span className="dom-va-tag">VA</span>}
                            </div>
                        );
                    })}
                </div>
            ...
            {domSignals.length > 0 && (
                <div className="dom-signals">
                    {domSignals.map((s, i) => (
                        <div key={i} className={`dom-signal-item ${s.tipo}`}>
                            {s.texto}
                        </div>
                    ))}
                </div>
            ...
            <style jsx>{`
                ...
                .dom-level.va { opacity: 0.9; }

                .dom-vol-ask, .dom-vol-bid {
                    display: flex;
                    align-items: center;
                    gap: 6px;
                }
                ...
                .dom-poc-badge, .dom-va-badge {
                    font-size: 7px;
                    font-weight: 900;
                    padding: 1px 3px;
                    border-radius: 3px;
                }
                .dom-poc-badge { background: rgba(245,158,11,0.3); color: #F59E0B; }
                .dom-va-badge  { background: rgba(99,102,241,0.2); color: #818CF8; }
                ...
                .dom-alert-icon { font-size: 20px; }
                .dom-alert-text { display: flex; flex-direction: column; gap: 2px; }
                .dom-alert-text strong { font-size: 11px; font-weight: 700; text-transform: uppercase; color: #fff; }
                .dom-alert-text span { font-size: 10px; color: rgba(255,255,255,0.7); line-height: 1.4; }

                /* ── Volume Profile ── */
                .dom-vp-section { margin-bottom: 12px; }
                .dom-vp-title { font-size: 9px; font-weight: 700; color: rgba(255,255,255,0.4); text-transform: uppercase; margin-bottom: 6px; letter-spacing: 0.5px; }
                .dom-vp-row {
                    display: flex;
                    align-items: center;
                    gap: 8px;
                ...
                .dom-vp-row.poc { background: rgba(245,158,11,0.08); }
                .dom-vp-row.va { background: rgba(99,102,241,0.04); }
                .dom-vp-price { font-size: 10px; font-family: monospace; color: rgba(255,255,255,0.7); width: 70px; flex-shrink: 0; }
                .dom-vp-bar-bg {
                    flex: 1;
                    height: 5px;
                    background: rgba(255,255,255,0.05);
                ...
                .dom-vp-bar-fill {
                    height: 100%;
                    background: linear-gradient(to right, rgba(59,130,246,0.4), rgba(99,102,241,0.6));
                    border-radius: 3px;
                }
                .dom-poc-tag, .dom-va-tag {
                    font-size: 7px;
                    font-weight: 900;
                    padding: 1px 4px;
                    border-radius: 3px;
                ...
                .dom-poc-tag { background: rgba(245,158,11,0.3); color: #F59E0B; }
                .dom-va-tag  { background: rgba(99,102,241,0.3); color: #818CF8; }
